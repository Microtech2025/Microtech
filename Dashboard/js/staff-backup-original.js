...
function showOfflineAlert() {
    if (offlineToastShown) return; // Prevent multiple alerts
    
    offlineToastShown = true;
    
    // Show persistent toast notification
    if (typeof showToast === 'function') {
        showToast('No internet connection detected. Some features may not work properly.', 'error');
    }
    
    // Show modal alert for critical operations
    ...
    alertModal.innerHTML = `
        <div class="modal-content" style="max-width: 400px;">
            ...
            <div class="modal-body">
                ...
                <div style="background: rgba(239, 68, 68, 0.1); padding: 12px; border-radius: 8px; margin: 16px 0;">
                    <p style="margin: 0; font-size: 14px; color: #DC2626;">
                        <strong>Limited functionality:</strong><br>
                        • Cannot save/load staff data<br>
                        • Cannot upload files<br>
                        • Cannot sync with Firebase
                    </p>
                </div>
            </div>
            <div class="modal-footer">
                <button class="btn btn-primary" onclick="retryConnection()" style="margin-right: 8px;">
                    <i class="fas fa-sync"></i> Retry Connection
                </button>
                <button class="btn btn-secondary" onclick="dismissOfflineAlert()">
                    Continue Offline
                </button>
            </div>
        </div>
    ...
    window.offlineAlertModal = alertModal;
}

function dismissOfflineAlert() {
    if (window.offlineAlertModal) {
        window.offlineAlertModal.remove();
        window.offlineAlertModal = null;
    }
}
...
async function retryConnection() {
    ...
    if (online) {
        ...
        // Retry loading staff data if needed
        if (allStaff.length === 0) {
            await loadStaffData();
        }
    } else {
        showToast('Still no internet connection. Please check your network settings.', 'error');
    }
}
...
function wrapFirebaseOperation(operation, operationName = 'operation') {
    return async (...args) => {
        // Check internet connection before Firebase operations
        if (!isOnline || !(await checkInternetConnection())) {
            showToast(`Cannot perform ${operationName} - no internet connection`, 'error');
            showOfflineAlert();
...
async function initializeStaffManagement() {
    try {
        ...
        // Enable new filtering and sorting
        enableDirectFiltering();
        enableDirectSorting();
...
function setupFilters() {
    ...
    if (departmentFilter) {
        console.log('Department filter found');
    ...
    if (viewFilter) {
        console.log('View filter found');
        viewFilter.addEventListener('change', handleViewChange);
    } else {
        console.log('View filter NOT found');
    }
    
    // Experience filter
    const experienceFilter = document.getElementById('experienceFilter');
    if (experienceFilter) {
        console.log('Experience filter found');
...
function setupSorting() {
    ...
    sortableHeaders.forEach((header, index) => {
        ...
        console.log(`Setting up header ${index}: ${field}`);
...
async function loadDepartments() {
    ...
    if (!isOnline || !(await checkInternetConnection())) {
        console.log('Cannot load departments - no internet connection');
        // Use fallback departments when offline
        ...
        populateDepartmentDropdowns(fallbackDepartments);
        return fallbackDepartments;
    }
    
    try {
        console.log('Loading departments from Firebase...');
        const departmentSnapshot = await db.collection('department').get();
        const departments = [];
        ...
        departmentSnapshot.forEach(doc => {
            ...
            departments.push({
                id: doc.id,
                name: dept.name || dept.departmentName || doc.id,
                ...dept
            });
        });
...
function populatePositionFilter() {
    if (!allStaff || allStaff.length === 0) {
        console.log('No staff data available for position filter');
        return;
    }
    
    // Get unique positions from current staff data
    const positions = [...new Set(allStaff.map(staff => staff.position).filter(position => position && position.trim() !== ''))];
    positions.sort();
    ...
    if (positionFilter) {
        ...
        console.log('Updated position filter dropdown');
    }
}

function populateStatusFilter() {
    if (!allStaff || allStaff.length === 0) {
        console.log('No staff data available for status filter');
        return;
    }
    
    // Get unique statuses from current staff data
    const statuses = [...new Set(allStaff.map(staff => staff.status).filter(status => status && status.trim() !== ''))];
    statuses.sort();
    ...
    if (statusFilter) {
        ...
        console.log('Updated status filter dropdown');
    }
}
...
function populateDepartmentDropdowns(departments) {
    ...
    if (departmentFilter) {
        ...
        console.log('Updated department filter dropdown');
    }
    
    // Update staff modal dropdown (add modal)
    const staffDepartment = document.getElementById('staffDepartment');
    if (staffDepartment) {
        // Keep the "Select Department" option
        staffDepartment.innerHTML = '<option value="">Select Department</option>';
        ...
        console.log('Updated staff department dropdown');
    }
    
    // Update edit staff modal dropdown
    const editStaffDepartment = document.getElementById('editStaffDepartment');
    if (editStaffDepartment) {
        // Keep the "Select Department" option
        editStaffDepartment.innerHTML = '<option value="">Select Department</option>';
        
        departments.forEach(dept => {
            const option = document.createElement('option');
            option.value = dept.name;
            option.textContent = dept.name;
            editStaffDepartment.appendChild(option);
        });
        
        console.log('Updated edit staff department dropdown');
    }
}
...
async function ensureDepartmentExists(departmentName) {
    ...
    try {
        ...
        const departmentQuery = await db.collection('department')
            .where('name', '==', departmentName)
            .get();
        
        if (departmentQuery.empty) {
            // Department doesn't exist, create it
            console.log(`Creating new department: ${departmentName}`);
            await db.collection('department').add({
                name: departmentName,
                createdAt: new Date(),
        ...
        // Don't throw error, just log it so staff saving isn't blocked
    }
}
...
async function loadStaffData() {
    ...
    // Check internet connection before loading
    if (!isOnline || !(await checkInternetConnection())) {
        console.log('Cannot load staff data - no internet connection');
        showOfflineAlert();
    ...
    try {
        console.log('Loading staff data from Firestore...');        // Create query for staff collection
...
function applyFilters() {
    ...
    const initialLength = allStaff.length;
    filteredStaff = allStaff.filter(staff => {
        ...
        if (currentFilters.status && currentFilters.status !== '') {
            if (staff.status?.toLowerCase() !== currentFilters.status.toLowerCase()) {
                return false;
            }
        }
        ...
        if (currentFilters.position && currentFilters.position !== '') {
            if (staff.position !== currentFilters.position) {
                return false;
            }
        }
        ...
        if (currentFilters.experience && currentFilters.experience !== '') {
            const experience = calculateExperience(staff.joiningDate || staff.hireDate);
...
function getSortValue(staff, field) {
    switch (field) {
        ...
        case 'hireDate':
            return new Date(staff.hireDate || 0);
        case 'salary':
            return parseFloat(staff.salary || 0);
        case 'experience':
            return calculateExperience(staff.joiningDate || staff.hireDate);
        default:
            return staff[field] || '';
    }
}
...
function handleSort(field) {
    ...
    showToast(`Sorted by ${field} (${currentSort.direction === 'asc' ? 'ascending' : 'descending'})`, 'info');
}
...
function updateSortIcons() {
    ...
    const currentHeader = document.querySelector(`[data-sort="${currentSort.field}"] .sort-icon`);
    if (currentHeader) {
        currentHeader.className = `fas fa-sort-${currentSort.direction === 'asc' ? 'up' : 'down'} sort-icon active`;
    }
}
...
function renderStaffList() {
    if (currentView === 'table') {
        renderTableView();
        // Re-enable sorting after table is rendered
        enableDirectSorting();
    } else {
        renderGridView();
    }
...
function renderTableView() {
    ...
    if (pageStaff.length === 0) {
        ...
        if (hasFilters) {
            tableBody.innerHTML = `
                <tr>
                    <td colspan="9" class="no-results-message text-center" style="padding: 40px;">
        ...
        <div class="staff-card">
            ...
            <div class="staff-card-body">
                ...
                <div class="staff-badges">
                    ...
                    <span class="experience-badge badge-${getExperienceLevel(calculateExperience(staff.joiningDate || staff.hireDate))}">
                        ${calculateExperience(staff.joiningDate || staff.hireDate)} years
                    </span>
                </div>
            </div>
...
// =============================================================================
// SIMPLIFIED FILTERING AND SORTING SYSTEM
// =============================================================================
...
function applyFiltersManually() {
    ...
    currentFilters.position = positionFilter ? positionFilter.value : '';
    ...
    showToast(`Filters applied: ${filteredStaff.length} staff members found`, 'success');
}
...
function clearAllFilters() {
    ...
    console.log('All filters cleared. Showing all staff:', filteredStaff.length);
...
function sortByColumn(columnName) {
    ...
    if (currentSort.field === columnName) {
        ...
        // New field, start with ascending
        currentSort.field = columnName;
        currentSort.direction = 'asc';
    }
    
    console.log('Sort settings:', currentSort);
    ...
    showToast(`Sorted by ${columnName} (${currentSort.direction === 'asc' ? 'A-Z' : 'Z-A'})`, 'info');
}

function enableDirectFiltering() {
    console.log('Setting up direct filtering...');
    ...
    // Filter dropdowns - immediate filtering
    const filterElements = [
        { id: 'departmentFilter', key: 'department' },
        { id: 'positionFilter', key: 'position' },
        { id: 'statusFilter', key: 'status' },
        { id: 'experienceFilter', key: 'experience' }
    ];
    ...
    filterElements.forEach(filter => {
        ...
        if (element) {
            element.addEventListener('change', (e) => {
                currentFilters[filter.key] = e.target.value;
                console.log(`${filter.key} filter changed to:`, e.target.value);
                applyFilters();
                renderStaffList();
            });
        }
...
function enableDirectSorting() {
    console.log('Setting up direct sorting...');
    
    // Add click events to table headers
    const headerMappings = [
        { selector: 'th:nth-child(2)', field: 'name' },       // Staff Member
        { selector: 'th:nth-child(3)', field: 'position' },   // Position
        { selector: 'th:nth-child(4)', field: 'department' }, // Department
        { selector: 'th:nth-child(5)', field: 'phone' },      // Phone
        { selector: 'th:nth-child(6)', field: 'email' },      // Email
        { selector: 'th:nth-child(7)', field: 'status' },     // Status
        { selector: 'th:nth-child(8)', field: 'experience' }  // Experience
    ];
    
    // Wait for table to be rendered, then add events
    setTimeout(() => {
        headerMappings.forEach(mapping => {
            const header = document.querySelector(`#staffTable thead ${mapping.selector}`);
            if (header) {
                header.style.cursor = 'pointer';
                ...
                // Add new listener
                header.sortHandler = () => sortByColumn(mapping.field);
                header.addEventListener('click', header.sortHandler);
                
                console.log(`Added sort handler for ${mapping.field}`);
            }
        });
    }, 500);
}
...
async function handleAddStaff(e) {
    ...
    // Check internet connection before saving
    if (!isOnline || !(await checkInternetConnection())) {
        showToast('Cannot add staff member - no internet connection', 'error');
        showOfflineAlert();
        return;
    }
    ...
    try {
        ...
        submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Adding Staff...';        // Collect form data
        ...
        // Ensure department exists in department collection
        await ensureDepartmentExists(staffData.department);
...
async function confirmDeleteStaff() {
    ...
    if (!isOnline || !(await checkInternetConnection())) {
        showToast('Cannot delete staff member - no internet connection', 'error');
        showOfflineAlert();
        return;
    }
...
function openEditStaffModal(staff) {
    ...
    // Refresh departments in case they've been updated
...
async function handleEditStaff(e) {
    ...
    // Check internet connection before updating
    if (!isOnline || !(await checkInternetConnection())) {
        showToast('Cannot update staff member - no internet connection', 'error');
        showOfflineAlert();
        return;
    }
    ...
    try {
        ...
        // Ensure department exists in department collection
        await ensureDepartmentExists(updatedData.department);
...
function updateSearchResultsInfo() {
    ...
    if (!searchResultsInfo || !filteredCountElement || !totalCountElement) return;
    ...
    // Update counts
    filteredCountElement.textContent = filteredStaff.length;
    totalCountElement.textContent = allStaff.length;
    
    // Show/hide the info based on whether filters are applied and results differ
...
async function processBulkUpload() {
    ...
    if (!isOnline || !(await checkInternetConnection())) {
        showToast('Cannot process bulk upload - no internet connection', 'error');
        showOfflineAlert();
        return;
    }
...
function testFiltering() {
    ...
    currentFilters.department = 'Teaching';
    applyFilters();
    renderStaffList();
}
...
window.applyFiltersManually = applyFiltersManually;
